...
import { fetchsongs,} from '../../store/songs-slice';  // 引入 Redux 中的异步操作 `fetchsongs` 和 `Habit` 类型
...
const songsStat: React.FC = () => {
   // 使用 `useSelector` 从 Redux store 中获取习惯数据状态
   const {isLoading, error } = useSelector((state: RootState) => state.songs);  // 获取习惯数据、加载状态和错误信息
   const dispatch = useDispatch<AppDispatch>();  // 获取 dispatch 用于派发 Redux actions

   useEffect(() => {
       dispatch(fetchsongs());  // 组件加载时调用异步 action `fetchsongs` 获取数据
